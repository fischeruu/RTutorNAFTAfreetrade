...
print_frechet_5 <- function(x){
  # a realistic value for lambda is 0.6
  theta =5
  0.6 *theta * x^(-1-theta) * exp(-0.6 *x^(-theta))
}
print_frechet_12 <- function(x){
  # a realistic value for lambda is 0.6
  theta =12
  0.6 *theta * x^(-1-theta) * exp(-0.6 *x^(-theta))
}
...
print_frechet3 <-function(a=0,b=2.4){
  t <- c(0.18,0.32,0.65)
  o <- 8.28
  y <- matrix(0,ncol=4,nrow=1001)
  y[,1] <- (0:1000)/1000*(b-a) +a
