...
library(yaml)
setwd("C:/Users/palimpalim/Desktop/Masterarbeit_Daten/R_masterarbeit/Eigenes/RTutor")
ps.name <- "NAFTA"; sol.file <- paste0(ps.name,"_sol.Rmd")
libs <- c("ggplot2","dplyr","gridExtra","reshape2", "shinyEvents") # character vector of all packages you load in the problem set

create.ps(sol.file=sol.file, ps.name=ps.name, user.name=NULL,libs=libs, stop.when.finished=FALSE 
         , extra.code.file = "functions.R" 
         , var.txt.file = "Data_Description_Exercise_1.csv"
         ,addons="quiz", table.max.rows = 40, signif.digits = 3
         )

show.shiny.ps(ps.name, load.sav=FALSE,  sample.solution=FALSE,
              user.name = "tf",
              catch.errors=TRUE, launch.browser=TRUE, signif.digits = 3, table.max.rows = 40)
stop.without.error()
```
#>
...
Here is a warm-up. Have you already heard about NAFTA? **Your task:** Solve this first quiz! In the course of this problem set there will be many quizzes in which you can test your knowledge and recap the information provided. Press the `check` button after having selected your answer choices. Do not worry: If there was a wrong answer, `RTutor` would tell you this and let you choose another answer option.

#< quiz "Have you already heard about NAFTA?"
parts:
  - question: 1. What does the acronym "NAFTA" stand for?
    sc: 
      - Northern Arabic Free Trade Area
      - North American Free Trade Agreement*
      - North African Free Trade Agency
    success: Great, the answer is correct!
    failure: This is not correct. Try again
  - question: 2. What are NAFTA's member states?
    sc:
    ...
    success: Great, the answer is correct!
    failure: This is not correct. Try again
  - question: 3. When did NAFTA come into force?
    sc:
      - April 1, 1900 
      - June 1, 1988
      - January 1, 1994*
...
In the following code chunk it is **your task** to press the `hint` button, so that you can see the exact instruction. Then enter the correct code. Finally press the `check` button to evaluate your solution. It is important that there is no hash key `#` in front of the code that you want to execute. On the other hand, comments cannot be executed and need a hash key `#` in front of them, for example `# Enter your command here`. 
```{r "1", optional=TRUE, eval=FALSE, points=0}
#< task
# Enter your command here
#>
#< hint
display("Write the code in a new line or delete the already written comment first. 
Then write or copy/paste the following two lines, finally press the check button:
...
In contrast to code chunks, quizzes need not be solved correctly in order to be able to advance to the next code chunks.

Now you know the basics tools to solve the problem set presented here.
...
#< quiz "Handling quiz"
parts:
  - question: 1. Which is the only source country we can find in this fraction of the data frame 'trade'?
    answer: Argentina
    success: Great, this is correct!
    failure: Unfortunately, this is not correct. Have a look on the column 'Source_C' and try again.
  - question: 2. What is the destination country in the fifth row of the data frame 'trade'? Solve this using the 'data' button as describe above.
    answer: Chile
    success: Great, this is correct!
    failure: This is not correct. You may try again if you want to, observing the column 'Dest_C' in the data frame 'trade'.
#>
...
```{r "1 c) 2", optional=TRUE}
#< task_notest
# Enter your code here
# do not forget to remove the hash keys `#`
#>
#< hint 
display("1&2: Have a look again at the code in the chunk above: how is the command unique() used there?
3: Use the function filter(trade, Tariffs2005 >....)")
#>

```

#< quiz "Exploring the data frame trade"
parts:
  - question: 1. How many different entries are there in the column `NAFTA_Dest`?
    answer: 4
    success: Great, this is correct!
    failure: Try to use the command group\_by(trade$NAFTA_Dest) in the code chunk above.
  - question: 2. What are these entries?
    mc:
    - Germany
    - World_beyond*
    - Mexico*
    ...
    success: Great, this is correct!
    failure: This is not correct. Try again.
  - question: 3. Which is the only country that imposed tariffs of more than 150% in 2005? Tariffs of 150% are indicated by the value 150 in the data frame.
    answer: Korea
    success: Great, your answer is correct!
    failure: Sorry, this is not the corret answer. Try to use the command filter(trade,Tariffs2005 \> 150)
#>
...
# Enter your command here
#>
levels(trade$Source_C)
#< add_to_hint
display("Use `levels()` just like you used `unique()` in part c), but in order to find source countries `Source_C` instead of sectors.")
#>
```
...
```{r "1 d) 2", fig.height=9, fig.width=12}
#< task_notest
printmap(trade, io)
#>
...
#< quiz "The countries covered by the available data"
parts:
  - question: 1. Which of the so-called BRICS countries is not included in the data?
    sc: 
      - Brazil
      - Russia*
      - India
    ...
    success: Great, the answer is correct!
    failure: This is not correct. Try again.
  - question: 2. Which of the so-called MINT countries is not included in the data?
    sc:
      - Mexico
      - Indonesia
      - Nigeria*
...
#< info "The column NAFTA_Dest"
The variable `NAFTA_Dest` has the four possible entries `Canada`, `Mexico`, `US` and `World_beyond`. It states which of the NAFTA members is the destination of the trade flow in question. Alternatively, `World_beyond` indicates that the destination is a country beyond the NAFTA area.
#>
...
#< info "Non-tradable sectors"
CP include sectors, whose outputs cannot be traded between different countries, due to two main reasons into their model that aims at measuring trade effects:
Firstly, there actually do exist sectors whose outputs are not really sold in a foreign country. Consider for example the land transport or the real estate sectors: Despite it is possible to construct examples how producers settled in a foreign country use the output of one of these sectors to produce a good, these cases are clearly the exceptions from the rule. Consequently it is absolutely legitimate to consider these sectors as non-tradable.   
Secondly, in the U.S. the share of non-tradable sectors in the production of tradable sectors was 34%, which proves the relevancy of considering non-tradable sectors in the value chain.
#>
...
pp1b
#< add_to_hint
display("Enter the command `filter(Tradable==1) %>%` into the indicated line.
do not forget to remove the hash keys `#`.")
#>
```
...
#< info "Doubling the arithmetic average"
The doubling of the arithmetic is imperative here, consider the arithmetic average ($\bar{x}$)'s definition: $\bar{x} = \frac{1}{n} \sum^n_{i=1} x_i$ where $n$ is the number of summands. Thus the sum of tariffs $\sum^n_{i=1} x_i$ stays the same when we leave out non-tradable sectors which have their tariffs $x_i$ set equal to zero. In the division step in contrast, we only divide by half of the amount of sectors (i.e. $n=20$ instead of $n=40$), which consequently doubles the result of the arithmetic average.  
#>
...
#< quiz "Understanding the bar plots"
parts:
  - question: 1. According to the correct calculation, what was the arithmetic average of tariffs on imports to Canada in 1993?
    answer: 5.98
    roundto: 0.05
    success: Very good!
    failure: Sorry, this is not correct. Have a look on the data frame 'pp1b'.
  - question: 2. If you compare this value to the plot of average tariffs, which part of bars in the bar plot should then correspond to tariffs in 1993? 
    sc:
    - The light blue part.
    - The dark orange part.
    - The whole bar.*
    - None of these options.
    success: Well done!
    failure: Sorry, this is not correct. Try to solve the question again with the help of the answer of question 1 and the bar plot 'pplot1b'.
  - question: 3. Does the dark orange part of the bar plot correspond to...
    sc:
    - the flat rate tariffs in 2005?
    - the average tariffs in 2005?*
    - the flat rate tariffs in 1993?
    - the average tariffs in 1993?
    - the difference between average tariffs in 2005 and in 1993?
    - the average tariff reduction between 1993 and 2005?
    success: Perfect!
    failure: Unfortunately, this answer is not the correct one. Compare the height of the bars in 'pplot1b' to the output of 'pp1b'.
  - question: 4. Does the light blue part of the bar plot correspond to...
    mc:
    - the flat rate tariffs in 2005?
    - the average tariffs in 2005?
    - the flat rate tariffs in 1993?
    ...
    failure: Sorry, you didn't find all correct answers. Use what you have learned in the preceding questions of this quiz.
#>
...
#>
trade %>%
  filter(Dest_C=="Mexico", Source_C=="USA", Tradable==1) %>%
  select(Sector, Tariffs1993, Tariffs2005, Trade_Vol) 
#< add_to_hint
display("Use the code chunk in Exercise 1.1 a).
Then add to the filter the condition `Tradable==1`,
and substitute in the selection part `Tradable` by `Trade_Vol`")
#>
```
...
pp2 <- trade %>%
  group_by(NAFTA_Dest) %>%
  summarize(Tariffs_weighted = sum(Tariffs1993*Trade_Vol)/sum(Trade_Vol),
            Tariffs_weighted_05=sum(Tariffs2005*Trade_Vol)/sum(Trade_Vol))
pp2
#< add_to_hint 
display("Use the functions `summarize()` and `group_by()` and
        sum(Tariffs1993*Trade_Vol)/sum(Trade_Vol) as well as
        sum(Tariffs2005*Trade_Vol)/sum(Trade_Vol)")
#>
```
...
#>
pplot2 <- ggplot(pp2) + 
  geom_bar(mapping=aes(x=NAFTA_Dest, y=Tariffs_weighted),stat="identity",  
           fill="lightblue", colour="lightblue4") + 
  geom_bar(mapping=aes(x=NAFTA_Dest, y=Tariffs_weighted_05),stat="identity", 
           fill="darkorange", colour="orangered")
pplot2
#< add_to_hint 
display("Which data frame do we want to plot? You created it in the last task.
The entries on the x-axis should be the values of `NAFTA_Dest`, shouldn't they?
The lengths of the bars (or y-values) should be weighted averages of tariffs. You created them in the last task, e.g. `Tariffs_weighted_05`.
Also confer the procedure in b).")
#>
```
...
```{r "1.2 a) 3", optional=TRUE}
#< task_notest  
# enter code freely here if you wish to
#>
```

#< quiz "Summary of Exercise 1.2 part a)"
parts:
  - question: 1. In which sector did Mexico reduce its tariffs on imports from the US most in the course of NAFTA? Write the answer in lowercase letters in just one word (e.g. retail).
    answer: textile
    success: Yes, this is correct!
    failure: Try again. The tariffs reduced from 18% to 0. The answer needs to be written in minuscule.
  - question: 2. In 1993, what was the value in billion dollars of the imports in this sector?
    answer: 2.51
    success: This is the right answer!
    failure: Maybe you did not consider that the answer should be given in billion dollars? Try again with the use of part c).
  - question: 3. What was the weighted average of Mexico's tariffs from global imports in 1993? Give the answer in percent.
    answer: 13
    success: Perfect!
    failure: This is not correct. Have another look at the table in part a) which gives the weighted tariffs.
  - question: 4. In all the bar plots in Exercises 1.1 and 1.2, how can we find the average tariffs in 1993?
    sc:
    - This is the value at the top of the whole bar.*
    - This is the value at the top of the dark orange bar.
    - This is the length of the light blue bar.
    success: Great, you are right!
    failure: Try again, this is not the right answer.
#>
...
#< info "The variable names 'GO_Source' and 'VA_Source' "
The data for gross output depends both on the country and on the sector involved - subsequently said to be country-sector specific.  
Note that the data frame `trade` is structured in a way that it provides data for country-country-sector specific constellations. Consider for example tariff data, which depends on the exporter, the importer and the sector.  
Now, if we called the variable for gross output simply `GO` or similar, there would be an ambiguity: Which country is concerned? Therefore, we name the variable `GO_Source` in order to make clear that it describes the gross output of the country `Source_C` in the given sector.  
The data frame `io` is constructed to host country-sector-sector specific data. So there would be an ambiguity, too. Still, this way of storing the data is preferred over the introduction of another data frame, in order to keep the structure of data simple, having only two main data frames.

The same holds for the variable `VA_Source`.
#> 
...
#>
trade %>%
    filter(Source_C=="Mexico",Sector=="Auto") %>%
    select(Source_C, Sector, GO_Source)
#< hint
display("1: We look for data from the Auto sector in Mexico.
2: What is the variable name for gross output?")  
#>
```
...
```{r "1.2 c) 3", optional=TRUE}
#< task_notest  
# enter code freely here if you wish to
#>
```

#< quiz "Summary of Exercise 1.2 from part b) on"
parts:
  - question: 1. Trade elasticities are...
    mc: 
    - not important for the model.
    - given in the data frame 'trade'.*
    - easily obtained from freely available data.
    - given in the plots in this exercise.
    - estimated in CP.*
    - an indicator for substitutability of goods.*
    success: Great, all answers are correct!
    failure: Not all answers correct. Try again.
  - question: 2. What is the percentage of output value of the Canadian mining sector that is added within the sector and not by inputs? In the free code chunk you may employ one of the methods which prevent the programme from giving out the same value 30 times.
    answer: 63.95469
    roundto: 1
    success: Great, you are right!
    failure: Try again, this is not the right answer.
  - question: 3. What was the weighted average of tariffs in non-NAFTA countries in 2005 for imports from the NAFTA countries? Give the answer in percent.
    answer: 3.492223
    roundo: 0.1
    success: Perfect!
    failure: Have another look at the table in part d).
  - question: 4. Which NAFTA member country had the lowest weighted average on imports from the other NAFTA members?
    sc:
    - Canada*
    - Mexico
    - USA
    ...
    failure: Have another look at the table in part d).
  - question: 5. According to the results above, is there a country that increased tariffs between 1993 and 2005 in a sector for one exporting country?
    sc:
    - Yes
    - No
    - The results do not answer this question.*
    success: This is correct! In fact, yes there were tariff increases, but this does not arise from the calculations above.
...
#< quiz "The WIOT table"
parts:
  - question: 1. In Australia, how much input into the mining sector did the agriculture sector provide in 1995 (in million US-$)?
    sc:
      - 3107
      - 12*
      - 8044
      - 820
    success: Yes, this is correct!
    failure: Try again. Remember the sectors involved.
  - question: 2. On the other hand, how many million dollars of input did the mining sector provide into the agricultural sector?
    answer: 29
    success: Great, your answer is correct!
    failure: Try again. Remember the two sectors involved.
  - question: 3. Which sector provided by far the most input into the agriculture sector? Write your answer in lowercase letters in just one word (e.g. retail).
    answer: agriculture
    success: Great. This is what is meant by 'strong diagonals' in the paragraph above.
    failure: Scan again the agriculture value chain. Is this a row or a column?
#>
...
#>
summarize(filter(io, Country=="Australia", Dest_S=="Food"), total=sum(IO_Coeff))
#< hint
display("Calculate the sum over the source sectors and keep the destination sector fixed.
Use 'io' ,`Dest_S`, `Australia` and 'IO_Coeff'.")
#>
```
...
```{r "1.3 a) 3", optional=TRUE, points=0}
#< task_notest
...
coeff <- coeff %>%
  filter(Country=="Australia", Dest_S=="Food") %>%
  mutate(WIOT_backwards = IO_Coeff/100 * GO_Source * (100-VA_Source)/100)

head(coeff, 6)
#< add_to_hint
display("1: The first factor is the i-o-coefficient divided by 100.
2: The second factor is the gross output
3: The third factor is one hundred minus the value added, divided by 100.
Just have a look at the output of the preceding code chunk to see the exact variable names.")
#>
```
...
```{r "1.3 b)", optional=TRUE, points=0}
#< task_notest
# type in the code you judge necessary here
# do not forget to remove the hash keys '#'
#>
#< hint
display("1: Hotels are the destination intermediate goods the real state sector provides. Use filter!
2: Idem.
3: Filter the data frame 'io' in a proper way and use the function arrange( ... , desc( ... )), where desc() stands for arranging the data frame in descending order.
4: Idem.")
#>
```

#< quiz "Input-Output Coefficients"
parts:
  - question: 1. Which percentage of the intermediate consumption in the 'Hotels' sector in Germany corresponds to the sector 'Real State'?
    answer: 15.9626100
    roundto: 0.1
    success: Great, your answer is correct!
    failure: Try again.
  - question: 2. What is the value of this consumption in million dollars? 
    answer: 6336.889
    roundto: 400
    success: Great, your answer is correct!
    failure: Try again. Take the large code chunk in part a) as an example.
  - question: 3. In Spain, which is the sector that contributes the highest percentage of intermediate consumption to a different sector? What is the corresponding destination sector? Write your answer in lowercase letters ('source sector , destination sector', e.g. retail , air transport).
    answer: construction , real state
    success: Great, you solved it correctly!
    failure: This is not correct. Try again if you want. Make sure to set the spaces and the comma right!
  - question: 4. What is the corresponding percentage?
    answer: 73.03787
    roundto: 0.1
    success: Correct!
    failure: this is not the correct answer. If you want to, try it again.
#>
...
io %>%
  group_by( Source_S ) %>%
  summarize(average=mean( IO_Coeff )) %>%
  arrange(desc(average)) %>%
  head(4)
#< add_to_hint
display("1: We want to use I-O coefficients. In which data frame can you find them?
2: We want to find out something about sectors that provide goods to other sectors. So are they destination or source sectors?
3: As it is written above in part 1, we want to use I-O coefficients.")
#>
```
...
#>
summarize(filter(io, Country=="Australia", Source_S=="Retail"), total=sum(IO_Coeff))
#< add_to_hint
display("Take the third code chunk in part a) as a starting point and replace the variable for the destination sector by the variable for the source sector. The sector in question should be 'Retail'.")
#>
```
...
#< info "Ricardian and Armington models"
Note again at this point, that the model we examine here in this RTutor problem set is developed by CP, who build upon a Ricardian trade model that EK develop. 

<br>

In the context of economic models, **Ricardian** is derived from David Ricardo (1772-1823), an important English political economist famous for his theory of comparative advantage.

This paper's model builds on the model that EK publish. They in turn take as a starting point the Ricardian trade model of Dornbusch et al. (1977) with two countries and a continuum of goods. Let us have a quick look at how Dornbusch et al. (1977) define their model:  
For the $n$ different goods (or sectors), they assume constant labour requirements $(a_1,a_2, \dots , a_n)$ in the home country, labour requirements $(a^*_1,a^*_2, \dots , a^*_n)$ in the foreign country and respective wages $\mathsf{w}$ and $\mathsf{w}^*$. Different labour requirements in the two countries are a way to represent different states of technology and thus comparative advantage. 
The home country will produce all the commodities $z$ in the interval $[0,1]$ for which domestic labour costs are not greater than foreign labour costs, namely $a(z)\cdot \mathsf{w} \leq a^*(z)\cdot \mathsf{w}^*$. A fundamental parameter for their model is $\omega \equiv \frac{\mathsf{w}}{\mathsf{w}^*}$, since the home country specializes on the commodities for which $\omega \leq {a(z)}/{a^*(z)}$. The foreign economy in turn specializes on the remaining commodities, i.e. for which ${a(z)}/{a^*(z)} < \omega$.

The demand is assumed to be homothetic, for example by Cobb-Douglas preferences. This means that the expenditure shares for all commodities are constant and that there are identical tastes in both countries. The total spending on goods from each economy gives its respective total income, which in turn determines the labour incomes $wL$ and $w^*L^*$. As a consequence, a higher spending on goods from the home country relative to the spending on goods from the foreign country raises $\omega$. This in turn lowers competiveness of the home country and leads to a smaller range of commodities produced domestically. Hence, it can be found an equilibrium value for $\omega$ which is an equilibrium for the whole model.


As EK point out, one characteristic feature of Ricardian models is that goods do not differ between countries. As a consequence, the specialization that countries undergo is due to comparative advantage or to differences in production technologies. Comparative advantage therefore is a key variable for the models of EK and of CP. We will come back to this topic in Exercises 2.1, 5 and 5.1.  

<br>


When it comes to analysing trade flows while employing CGE and gravity characteristics, there is an alternative to Ricardian models. Actually, in the literature **Armington** models are more standard. In those models, goods are unique due to their country of origin. Therefore, goods are always bought from all countries, the so-called 'love for variety'. Still their prices determine the respective demand for the goods, see Piermartini and Teh (2005). The Armington elasticity additionally determines to which extent goods from different countries can be substituted. Piermartini and Teh (2005) also state that in CGE models with this Armington assumption, tariff changes tend to lead to significant changes in trade. 
...
#< quiz "Trade models"
parts:
  - question: 1. CGE may stand for... 
    mc:
    - Computational General Equilibrium.*
    - Computable General Equilibrium.*
    - Computer Global Engineering.
    success: These are the correct answers!
    failure: Try again. Not all correct answers are found.
  - question: 2. They are...
    mc:
    - tools designed to calculate how an economy would react to external changes, using some kind of elasticity.*
    - simple models.
    - particularly complex in the case of CP.
    - often criticized as highly complex black boxes, while CP address this issue.*
    success: These are the correct answers!
    failure: Try again. There is still something wrong.
  - question: 3. Who was David Ricardo?
    mc:
    - A Mexican businessman.
    - A former NAFTA chairman.
    - A classical economist.*
    - The pioneer of comparative advantage theory.*
    - The pioneer of absolute advantage theory.
    success: These are the correct answers!
    failure: Try again. Not all correct answers are found.
  - question: 4. Which characteristics distinguish Ricardian models (R) and Armington models (A)?
    mc:
    - In A goods are unique due to their origin, whereas in R goods are substitutable across countries.*
    - In R goods are unique due to their origin, whereas in A goods are substitutable across countries.
    - In R, buyers search for the lowest price globally, independent from the good's country of origin.*
    - In A, buyers search for the lowest price globally, independent from the good's country of origin.
    success: These are the correct answers!
    failure: Try again. Not all correct answers are found.
  - question: 5. In Ricardian models...
    sc:
    - labour requirements (technology) change and can be solved for in order to find an equilibrium.
    - relative wages change and can be solved for in order to find an equilibrium.*
    - total income of an economy is constant.
    - None of these.
    success: Perfect, this is correct!
    failure: Try again. 
#>
...
#< info "Gravity Equations"
Anderson (1979) provides a theoretical foundation for gravity equations. He defines an ordinary version of a cash flow for one good from country $i$ to country $j$ due to trade as 
$$M_{ij}=\alpha Y^\beta_i Y^\gamma_j N^\delta_i N^\epsilon_j d^\mu_{ij} \cdot U_{ij}.$$ 
Hereby, $\alpha,\beta,\gamma,\delta,\epsilon,\mu$ are constants, $Y_k$ is the income in country $k$, $N_k$ the population of country $k$ and $d_{ij}$ is the distance between the two countries $i$ and $j$. Lognormally distributed $U_{ij}$ is the error term with $\mathbb{E}(\ln U_{ij})=0$.


An extended and still simple form, the so-called **Pure Expenditure System** model, has the following assumptions:

- Each country is specialized in one good.

- There are no tariffs and no transport costs.

- $b_j$ is the fraction of income spent on the product of country $j$.

- $b_j$ is the same for all countries.

- Prices are constant at equilibrium.

- Choose the units such that prices constantly equal 1.

- Income in country $i$ is given by $Y_i$

- Income is assumed to be equal to the sales, thus $Y_i \equiv b_i \left ( \sum_j Y_j \right )$, or equivalently $b_i = \frac{Y_i}{\sum_j Y_j }$.

Hence, the imports of the only good from country $i$ to country $j$ are given by 
$$M_{ij}=b_i Y_j= \frac{Y_i Y_j}{\sum_j Y_j}.$$

Note the similarities of both the meaning and representation of $b_i= \frac{Y_i}{ \sum_j Y_j }$ in Anderson's model (1979) and $\pi^j_{ni}= \frac{X^j_{ni}}{X^j_n}= \frac{X^j_{ni}}{\sum_h X^j_{nh}}$ in CP (see Exercise 2.5 b): 

$b_i$ and $\pi^j_{ni}$ both stand for the share of spending on goods from country $i$. Both are calculated as the ratio of one country's income over global income respectively as expenditure over global expenditure.   
Differences in the parameters and indexing of the two formulae arise from the model specifications, i.e. from CP allowing for country-specific expenditure shares and for several countries producing one type of goods. 
...
#< quiz "Questions on Gravity Equations"
parts:
  - question: 1. Which of the statements concerning gravity equations are correct?
    mc:
    - A country's expenditure shares can be calculated in the presented models as the ratio of sectoral spending on goods from one country over total spending on goods from this specific sector.*
    - In Anderson's (1979) Pure Expenditure System model consumption preferences are the same across countries.*
    - In CP, there are country-specific consumption preferences.*
    - In the basic gravity model the imports from one country to another depend on both countries' income and population, as well as the distance between both countries.*
    success: These are the correct answers!
    failure: Try again. Not all correct answers are found.
  - question: 2. How does the term 'gravity' fit to the functionality of Anderson's (1979) basic model?
    sc:
    - The model is mainly used in Physics in the gravity area.
    - Similar to Physics, just figuratively. The more economic weight two countries have (i.e. population and income) and the lower their distance, the higher is the respective attraction (i.e. trade flows).*
    success: Perfect, this is correct!
    failure: Try again. 
#>


#< info "Other innovations of CP"
- CP claim for their model that their approach of sectoral linkages between tradable and non-tradable sectors is an innovation compared to other Ricardian models in the literature.

- They also claim that their method of estimating trade elasticities (see Exercise 5.1) is an innovation.

- On the one hand, CP do address the non-symmetry of trade flows, whereas it is often an issue in the literature that symmetry is assumed. Helpman et. al (2008) state that this assumption of symmetry in general does not reflect the data and produces biased estimates. So CP avoid biases in this case. 

- On the other hand, in their estimation of trade elasticities (see Exercise 5.1), CP remove data for country pairs without trade flows between them. Helpman et. al (2008) address this issue of leaving out data of zero trade flows. They show that this standard procedure in the literature produces biased estimates.  
#>
...
#< info "Frechet distribution"
The Frechet probability distribution is continuous. It is often used in extreme value theory, in which it is one of the three **generalized extreme value distributions**. In this paper's model, according to the Frechet distribution, the cumulative distribution function of the efficiency of producing good $\omega^j$ is given by $\mathbb{P}(z^j_n(\omega^j)\leq z) = exp(-\lambda^j_n z^{-\theta^j})$. You can find its probability density function below. Note that in the literature, there are sometimes also used different parameters in the Frechet distribution, like for example $s$ instead of $\lambda$ with $s=\lambda^{-1/\theta}$.
#>
...
#< quiz "Theta and Lambda"
parts:
  - question: 1. Which of the following statements for 'lambda' are true?
    mc:
    - They need to be estimated in CP's model.
    - They are the location parameters of the Frechet distribution.*
    - They are the shape parameters of the Frechet distribution.
    - They quantify the comparative advantage.
    - They quantify the absolute advantage.*
    - They determine the average productivity in a sector.*
    success: Yes, you got it right!
    failure: Not all answers are correct. Try again!
  - question: 2. Which answers hold for the 'theta'?
    mc:
    - They need to be estimated in CP's model.*
    - They are the location parameters of the Frechet distribution.
    - They are the shape parameters of the Frechet distribution.*
    - They quantify the comparative advantage.*
    - They quantify the absolute advantage.
    - They determine the concentration of productivity in a sector.*
    success: Yes, you got it right!
    failure: Not all answers are correct. Try again!  
#>
...
```{r "2.2) 1", points=0, optional=TRUE}
#< task_notest
...
```{r "2.2) 2", points=0, optional=TRUE}
#< task_notest
# You may freely use this code chunk to solve the quiz.
#>
#< hint
display("Use the function print_frechet2() and for example the parameters a=0.9, b=1")
#>
```


#< quiz "Frechet distribution of Production Technologies/Efficiencies"
parts:
  - question: 1. Which of the statements concerning production efficiency are correct?
    mc:
    - Portugal has always a higher production efficiency than the USA and the Japan.
    - The US has always a higher production efficiency than Portugal and the Japan.
    - It is possible that any of the countries has the highest efficiency.*
    - It is more likely that the US has the highest efficiency in producing a certain good than that Portugal has the highest efficiency.*
    - It is very likely that the countries have an efficiency which is lower than 1.6.*
    - It is very likely that the countries have an efficiency which is higher than 1.6.
    success: These are the correct answers!
    failure: Try again. Not all correct answers are found.
  - question: 2. What is the value of production efficiency that Portugal exceeds with a probability of 50 percent? Round to 0.01.
    answer: 0.68
    roundto: 0.01
    success: Perfect, this is correct!
    failure: In other words, you should try to find the median of the red line.
  - question: 3. What is the value of production efficiency that Japan exceeds with a probability of 50 percent? Round to 0.01.
    answer: 0.98
    roundto: 0.01
    success: Perfect, this is correct!
    failure: In other words, you should try to find the median of the blue line.
  - question: 4. What is the probability that the Portugal production efficiency of a specific good exceeds 0.55. Give the answer rounded to whole percent.
    answer: 98
    roundto: 1
    success: Great, you solved it!
    failure: Sorry this is not correct. Maybe you did not write the value in percent? Or maybe you gave one hundred minus the wanted value?
#>
...
#< info "The function uniroot()"
The function `uniroot(f, interval)` numerically finds the root of a function which is given as an argument. The root often is zero. Consider for example the function `2*x`. We want to (numerically) find the x-value for which the function value is equal to 3. In order to be able to use the function `uniroot()` which finds the x-value for which the function value is equal to 0, we transform the function to `2*x - 3` and evaluate this function with `uniroot()`. Therefore, as arguments to `uniroot()`, we give the function in x `function (x) 2*x - 3`. Another argument is the interval on which we try to find the root, here we search the interval from 0 to 5. 


Hence as an example, if you want to evaluate the code  
> uniroot(f= function (x) 2*x - 3, interval= c(0,5) )$root

you obtain the result
> [1] 1.5

which is the right solution, since the x-value which gives a function value of 3 is 1.5.  
#> 
...
```{r "2.2) 3", points=0, optional=TRUE}
#< task 
...
```{r "2.2) 4", points=0, optional=TRUE}
#< task 
# the price for the Portuguese product:
(0.23*3.25) / 0.68
# the price for the Japanese product:
...
```{r "2.2) 5", points=0, optional=TRUE}
#< task
...
```{r "2.2) 6", points=0, optional=TRUE}
#< task
# replace the '???' by the right function
#uniroot(f = function (x) ??? , interval= c(0,2))$root
#>
uniroot(f = function (x) exp(-0.58*x^-8.28) - 0.1, interval= c(0,2))$root
#< hint
display("You need to use a formula as in the code chunk above, replacing -0.028 and 0.9 by the right values.")
#>
```
...
```{r "2.2) 7", points=0, optional=TRUE}
#< task 
# the price for the Portuguese product:
# ???
# the price for the Japanese product:
...
(0.78*1) /0.85
#< hint
display("You may copy-paste the code of the second code chunk where we calculated the price of good omega. Then simply replace the production efficiencies by the new values. The production efficiencies are in the formulas' denominator.")
#>
```
...
#< info "The basic concept of production technology"
Since the basic concept of production technology is neither central to CP nor to this RTutor problem set, it is not explained here. If you are interested into this part you may consider CP in the subchapter 3.1.3 on pages 7f. There you can see in detail how production technology and the elasticity of substitution enter into the model. 
The result there is, that production technology depends strongly on efficiency of production $z^j_n(\omega^j)$.
Together with the information on distributions of the $z^j_n(\omega^j)$ in Exercise 2.1, we find that production technology is heterogeneousous across countries and sectors and has a probabilistic representation. This is one of the technical features of the model EK develop and which CP extend.

Within this problem set we shall focus on the main aspects of the model. That is, we focus on $z^j_n(\omega^j)$ in the course of this RTutor problem set instead of production technologies.
#>
...
#< quiz "2.3 a)-b)"
parts:
  - question: 1. In the model, what are sources of income for the households?
    mc:
    ...
    failure: Try again, you didn't choose all the correct answers!
  - question: 2. Select the correct answers.
    mc:
    - Labour is a production factor.*
    - Wages do contribute to unit costs.*
    - The production efficiency is only country-dependent.
    - Both the share of value added and the share of intermediate consumption are important for the input bundle costs*
    - Input bundle costs of one sector depend on the unit price of the composite intermediate good of only that same sector.
    - The price at which firms sell their products depends on competition on the respective market.
    - The efficiency of producing an intermediate good depends on the country, on the sector and on the intermediate good which is produced.*
    - There are 40 intermediate goods in each sector.
    - There are 40 intermediate goods in total.
    success: Great, you found all the good answers!
    failure: Not quite. Try again if you want.
#>
<br>
...
```{r "2.3) 2", optional=TRUE, points=0}
#< task
#trade %>%
#  filter( ??? ,Tradable==1) %>%
...
#>
trade %>%
  filter(Dest_C=="Japan", Source_C=="Portugal",Tradable==1) %>%
summarize(average_tariff=mean(Tariffs1993)) 
#< hint
display("1: Which is the destination country and which is the source country?
2: You want to calculate the mean over the tariffs of 1993.")
#>
```
...
#< info "The relation between the price of the composite intermediate good and the prices of the intermediate goods"
$P^k_n$ is the unit price of the composite intermediate good. As its name says, the composite intermediate good is composed of all the intermediate goods $\omega^j$.  
CP assume to have a constant elasticity of substitution (CES) $\sigma^j > 0$ across intermediate goods within sector $j$. Then they calculate $P^k_n$ as:
$$P^j_n = \left[ \int p^j_n (\omega^j)^{1-\sigma^j} d \omega^j  \right]^{ \frac{1}{1-\sigma^j} }.$$
#>
...
#< quiz "2.3 c) -d)"
parts:
  - question: 1. In the model, what are components of prices for intermediate goods?
    mc:
    - The cost of an input bundle*
    - Profit margins
    - The production efficiency of the intermediate good which is priced*
    - Iceberg costs*
    - The country's production efficiency
    - Tariffs*
    success: You got it right!
    failure: Try again!
  - question: 2. Can households consume intermediate goods as well?
    sc:
    - Yes*
    - No
    success: Correct!
    failure: In fact, intermediate goods can be materials for further production and also final consumption goods.
  - question: 3. 'Intermediate good' is only an abbreviation for 'composite intermediate good'. True or false?
    sc:
    - True
    - False*
    success: Correct!
    failure: No, see the info box "The price of the composite intermediate good" for the explanation.
#>
...
#< info "The data frame `temp_2.4.b` for the following calculation"
`temp_2.4.b` basically is an excerpt of `trade`, extended by a row where Indonesia is both source and destination country. Note that, in this row, `Trade_Vol` has a slightly different meaning than what we are used to: Instead of inter-country trade, it presents the domestic sales, the value of goods that are both produced and consumed/used in Indonesia. 
```
> temp_2.4.b 
   Sector        Source_C    Dest_C Tariffs1993 Trade_Vol
1    Wood       Argentina Indonesia       30.00  0.000000
2    Wood       Australia Indonesia       22.61  1.243621
3    Wood         Austria Indonesia       20.00  0.008923
4    Wood          Brazil Indonesia       20.00  0.010097
5    Wood          Canada Indonesia       19.42  0.160925
6    Wood           Chile Indonesia       12.51  0.000000
7    Wood           China Indonesia       23.89  2.020409
8    Wood         Denmark Indonesia       27.19  0.091932
9    Wood         Finland Indonesia       30.00  0.001028
10   Wood          France Indonesia       25.86  0.005500
11   Wood         Germany Indonesia       22.59  1.049460
12   Wood          Greece Indonesia       12.51  0.000000
13   Wood         Hungary Indonesia       12.51  0.000000
14   Wood           India Indonesia        7.38  0.037967
15   Wood       Indonesia Indonesia        0.00 10.682365
16   Wood         Ireland Indonesia       12.51  0.000000
17   Wood           Italy Indonesia       29.84  0.640290
18   Wood           Japan Indonesia       25.59 12.723970
19   Wood           Korea Indonesia       20.10  0.246218
20   Wood          Mexico Indonesia       30.00  0.000000
21   Wood     Netherlands Indonesia       20.08  0.055712
22   Wood     New Zealand Indonesia       19.05  1.022896
23   Wood          Norway Indonesia       20.18  0.060404
24   Wood        Portugal Indonesia        7.60  0.208663
25   Wood     SouthAfrica Indonesia       12.51  0.000000
26   Wood           Spain Indonesia       24.43  0.035073
27   Wood          Sweden Indonesia       20.13  0.063156
28   Wood          Turkey Indonesia       12.51  0.000000
29   Wood              UK Indonesia       16.46  0.048769
30   Wood             USA Indonesia       18.88  5.361178
31   Wood Remaining World Indonesia       24.87  8.011868
```
#>
...
#>
temp_2.4.b <- temp_2.4.b %>%
  mutate(Expenditure = Trade_Vol*(1+ Tariffs1993/100))  %>%
  mutate(Exp_Shares = 100* Expenditure/sum(Expenditure))
temp_2.4.b
#< add_to_hint
display("1: See the formula above how you should calculate expenditures. But pay attention to the way the tariffs are presented: in percent!
2: Try to divide 'Expenditure' by the sum of 'Expenditure'")
#>
```
...
#>
#< hint
display("1: \"inputs\" shall be the data frame of input-output coefficients for Mexico with \"Auto\" as destination sector. You need to filter 'io' according to these criteria.
2: \"val_add\" is the share of value added for Mexico with \"Auto\" as destination sector. Try to filter the data frame according to the sector.")
#>
inputs <- filter(io, Dest_S=="Auto",Country=="Mexico")
  # Dest_C is arbitrary
val_add <- filter(trade, Source_C=="Mexico",Dest_C=="UK", Sector=="Auto")$VA_Source  
```
...
#>
#< hint
display("1: The first factor should be the input-output coefficient.
2: The second factor should be one minus the share of value added within the sector.")
#>
gamma_kj_n <- mutate(inputs, Gamma_kj_n=IO_Coeff/100 * (100-val_add)) # gamma_kj_n in %
gamma_kj_n
## the sum of input costs equals one minus the share of value added within the sector
100 - sum(gamma_kj_n$Gamma_kj_n)
val_add
```
...
#< quiz "Needed Parameters"
parts:
  - question: 1. The share of intermediate consumption and the input-output coefficient
    mc:
    - lie both between (-100) and (+100).
    - lie both between 0 and 1.*
    - have another image set (range where they could have values).
    - are related, as the I-O coefficient is at least as high as the intermediate consumption share.*
    - are related, as the I-O coefficient is at best as high as the intermediate consumption share.
    - these two parameters are not related.
    success: These are the correct answers!
    failure: Try again. Not all correct answers are found.
  - question: 2. The value added takes only values in the range between
    sc:
    - (-1) and (+1).
    - 0 and 1.
    - 0 and 100.
    - none of the previous.*
    success: Yes! You understood that the value added and the share of value added are different concepts.
    failure: I'm sorry, this is not correct. Pay attention to the difference between value added and the share of value added!
  - question: 3. What does sum up to 1?
    mc:
    - The intermediate consumption shares of one country (source sector fixed).
    - The intermediate consumption shares of one country (destination sector fixed).
    - The expenditure shares in one sector (source country fixed).
    - The expenditure shares in one sector (destination country fixed).*
    - The I-O coefficients of one country (source sector fixed).
    - The I-O coefficients of one country (destination sector fixed).*
    - The intermediate consumption shares of one country (source sector fixed) plus the value added share of that country in the source sector.
    - The intermediate consumption shares of one country (destination sector fixed) plus the value added share of that country in the destination sector.*
    success: Perfect, you found all correct answers!
    failure: Sorry, not all answers are correct.
#>
...
#< quiz "Alphas"
question: The final consumption share reflects the behaviour of...
sc: 
  - the firms only.
  - the firms and the government.
  - the firms, the government and the households.
  - the government only.
  - the households only.*
success: Yes, indeed!
...
#< info "Expenditure shares, Ricardo and gravity"
As CP and EK state, equation **(3)** visualizes $\pi^j_{ni}$'s similarity to a gravity equation as it is mentioned in the info box "Gravity Equations" in Exercise 2 c). Namely, both here and in the standard gravity equation, trade between two countries depends on the importer's total expenditure $X_n$ and on barriers between two countries $\kappa^j_{ni}$.

EK derive this gravity-type property from the basic framework of their model, hence CP also do so since they build on this model. Two findings in EK's model are particularly important for this link between the Ricardian framework and the gravity property:

- Let us consider a given good. Then for an importing country $n$, the **distribution of prices it actually pays** for this good from an exporter $i$ is always the same, independent from the exporting country $i$. 

- $\pi^j_{ni}$ actually is the probability that country $i$ offers the good $\omega^j$ at the lowest of all prices in country $n$. This is also the share of sector $j$ goods which the importer $n$ purchases from the exporter $i$, simply because there are infinitively many goods per sector in their model.
#>
<br>


Now it is time to apply what you have learned so far in this exercise, so it is **your task** to solve the following quiz. You may use the free code chunk if you want to.
...
#>
#< hint 
display("1: You may use this code as you judge it adequate.")
#>

```

#< quiz "Expenditure and Expenditure Shares"
parts:
  - question: 1. How many million dollars did the Indonesian spend on products from the wood sector in 1993? Use the output and/or the data frame 'temp_2.4.b' from Exercise 2.4 b).
    answer: 51.6
    roundto: 1
    success: Perfect, this is correct!
    failure: This is not correct. Try again. We are looking for the total expenditure. You may for instance take one country's expenditure and divide it by its expenditure share (not in percent).
  - question: 2. Let us consider the following. First, in 1993, France had an expenditure in the wood sector of roundabout 10 billion dollars. Second, the Indonesian population was about 180 million people in 1990. So realistically, the Indonesian expenditure as in question 1 appears... 
    sc:
    - probably too low.*
    - reasonable.
    - probably too high.
    success: This is probably correct.
    failure: Have another guess, it seems that your answer is not likely. French expenditure was roundabout 10000 million dollars.
  - question: 3. According to the terms it contains, equation 3 looks similar to another equation. Also it is linked to this equation. What is that equation's number?
    answer: 2
    success: Great, you detected it.
    failure: Sorry, Sorry. This is not correct.
  - question: 4. According to the formulas above, how many parameters of the Frechet distributions of production efficiencies are necessary for calculating expenditure shares?
    mc:
    - 0*
    - 1
    - 2
    ...
    - 33
    success: Great, this is correct!
    failure: Sorry, you did not find all correct answers. There are two formulas above for the expenditure share calculation. Also consider that N=31 in CP's model.
#>
...
#< info "The households' income in CP"
The income's formula $I_n=\mathsf{w}_n L_n + D_n+ R_n$ can be discussed controversially. It is not clear why trade deficits $D_n$ should increase income. CP do not mention this issue explicitly while they also call the variable $I_n$ the **final absorption** in country $n$. Furthermore they take the assumption that households spend all their income, or in other words, that all the final absorption is spent. So CP's indirect justification for trade deficits having a positive impact on income is, that an amount equal to the trade deficit is received by transfers from the rest of the world. Since in their model there is no investment income, they indirectly assume that all countries' current accounts are zero.    
#>
...
In order to recap the latter part of this exercise and connect it with Exercise 2.4 b), **your task** now is to answer the following quiz. If you want to, you may also use the following free code chunk. If you decide to do so, you first need to read in the needed data.
```{r "2.5) 1"}
#< task
trade <- read.csv("trade.csv")
io <- read.csv("io.csv")
...
The following chunk you can use freely.
...
#>
#< hint 
display("5: Note that in the column 'Trade_Vol' in the data frame 'trade' the values are given in million dollars without tariffs. So maybe you do not need to perform difficult calculations. Just summarize the data from the adequate data frame you obtain by filtering 'trade' in order to obtain the two components of a trade deficit.")
#>
...
#< quiz "Rest of 2.5"
parts:
  - question: 1. In the model, the savings rate is equal to how many percent?
    answer: 0
    success: Yes, you are right!
    failure: Sorry, this is not correct. Try again. You find the necessary information in this exercise.
  - question: 2. In the model, the unemployment rate is equal to how many percent?
    answer: 0
    success: Yes, you are right!
    failure: Sorry, this is not correct. Try again. You find the necessary information in this exercise.
  - question: 3. Why, in this model, do tariffs benefit households?
    sc:
    - Tariffs lead to a lower consumer price index.
    - Tariff income directly increases the income of the households.*
    - Tariffs increase domestic production because exports are less expensive.
    success: Perfect, this is right!
    failure: Sorry, this is not yet correct. Maybe you missed one of the two good answers?
  - question: 4. Give the trade deficit of Canada in the Mining sector in million dollars. A trade surplus is indicated with the minus sign.
    answer: -8090.586
    roundto: 1
    success: Great, you solved this difficult task!
    failure: Sorry, this is not correct. Maybe you confused the algebraic sign of the result?
  - question: 5. Give the trade deficit of Canada in the Air Transport sector in million dollars. A trade surplus is indicated with the minus sign.
    answer: 0
    success: You got it right! Air transport is a non-tradable sector, consequently there is no trade deficit possible.
    failure: Did you consider that Air Transport is not a tradable sector?
#>
...
#< quiz "The Equilibrium"
parts:
  - question: 1. What are reasons why CP solve for an equilibrium in relative changes rather than for an absolute equilibrium?
    mc:
    - Less variables need to be estimated which reduces calculation effort and possible errors.*
    - All the changes of global trade between 1993 and 2005 are considered for the final equilibrium.
    - Only changes in global trade that arise from changes in tariffs are obtained from the model.*
    - The calculated equilibrium can be verified easily by real trade data from 2005.
    - Both parameters of the Frechet distributions of production technologies need not be estimated.
    success: Perfect, these answers are correct!
    failure: This is not correct, try again.
  - question: 2. The equilibrium of relative changes specifies equations for the calculation of....
    mc:
    - changes of tariffs.
    - total expenditures in 1993.
    - total expenditures in 2005.*
    - changes of expenditures.
    - trade balance in 1993.
    - trade balance in 2005.*
    - trade balance changes.
    - bilateral trade shares in 2005.
    - changes in bilateral trade shares.*
    - price indices in 2005.
    - price index changes.*
    - input bundle costs in 2005.
    - input bundle cost changes.*
    success: Perfect, you found the correct answers!
    failure: Not all of the five correct answers are found. Try again.
#>
...
#< info "The balance between transparency and calculation efficiency"
In this info box, consider the following three options for more efficient calculation than we show it here in this RTutor problem set:   

The first option is performing matrix calculations as CP do it. So they find expenditures (equation **(9)**) much faster by inverting a matrix. Generally, matrix calculations are faster than data frame calculations.   The second method is using `R` packages like `nleqslv` or `BB` for solving systems of nonlinear equations. At least they would solve faster for expenditures (equation **(9)**) and make the inner `while` loop unnecessary - you can see the loop in the note in Exercise 3.3 and in the function `equation9()` below. Furthermore, finding the equilibrium of costs and prices (equations **(6)** and **(7)**) would be faster - you find a slow calculation procedure in Exercise 3.1 an in the function `equations67()` below.  
The third possibility is to work with interfaces between the generalist `R` and more specialist programming languages like `AMPL`, which help to solve in a more comfortable and faster way for systems of equations. 

A drawback which all these methods have in common, is that they cannot be performed in the course of this RTutor problem set. The reasons for this are twofold: First, with all of them it is hard to keep track with what is going on, so they are not suitable for our purpose here of getting easy access to the core of the work. Second, despite their relative speed, they would still take far too much time on a standard processor, because the CGE model here solves for several thousand parameters at the same time because of the linkages between all sectors and countries.  

Hence, for the sake of transparency in the code, we stick to the data frame representation with rather basic commands.   
#>
...
#< info "The methodology of restricting the model within this RTutor problem set"

First, we need to adapt the restricted data so that the model can still be applied. Explicitly, since sectors are removed, we have to take some assumptions in order to define what shall happen with the input-output coefficients. Additionally, how should we proceed with the trade flows with countries that we omit?

Concerning the trade flows, three methods could be pursued:
- as CP, we can aggregate the countries that do not explicitly play a role in the model in a constructed `Remaining World` and estimate values for its data. However, since we would like to keep the number of calculation low, we do not follow this method.
- we can adjust the expenditure shares by keeping constant the ratio of contribution to the expenditure of countries which stay in the restricted model, or
- we can view the imports from the discarded countries as produced within the destination country and similarly view the exports to discarded countries as consumed within the source country within the restricted model.

Under the assumption that the values of exports and imports between a pair of countries are of the same order/ similar, we can cancel out trade flows between a country within the restricted model and a country outside of the restricted model. This is the reason why we opt for the third method.


When it comes to the adaptation of input-output coefficients, similar methods can be found. Despite this, we opt for keeping constant the ratio between the sectors we keep in the model. The reason is that the input-output table already has a strong diagonal, meaning that for all sectors materials from the same sector are very important. If we assigned the input from the remaining sectors to the destination sector this would overweight the importance of the own sector's inputs and deteriorate the beneficial interdependence of the sectors. 
...
#< quiz "Exercise 3"
parts:
  - question: 1. What is the nature of the data frame 'dta'?
    sc: 
    - It is exactly the data frame 'trade'.
    - It is the data frame 'trade' extended by rows where source and destination countries are identical.
    - It is the data frame 'trade', extended by rows where source and destination countries are identical and extended by columns where important starting values for the model are already calculated.*
    - None of the former.
    success: Yes, indeed!
    failure: Sorry, this is not the correct answer.
  - question: 2. Will we calculate all the steps to find the equilibrium in relative changes?
    mc:
    - Yes, because we want to proof CP right.
    - No, because calculations in data frames are far too time consuming.*
    - No, because the model is still too complex for the scope of a RTutor problem set.*
    - Yes, because it is our aim in a RTutor problem set to show every detail of CP's publication.
    - No, because the numerical calculation has several loops integrated, so that even in matrix calculations it would take far too long to be able to show all of the steps here.*
    - No, because we do not have all the needed data here.
    success: Great, you understood the main issues of the calculation!
    failure: Sorry, not all correct answers have been found. Try again!
  - question: 3. Consider two countries with a rather different state of the economic development, for example Mexico and the U.S. What would CP's guess for the respective parameters 'w_hat' at the beginning of the iteration be?
    sc:
    - They would randomize the guess.
    - They would guess that w_hat for the U.S. is significantly higher than w_hat for Mexico.
    - They would guess that w_hat for the U.S. is a little smaller than w_hat for Mexico.
    - They would assume that w_hat equals 1 for both countries.*
    success: Great, this is the correct answer!
    failure: Sorry, this is not correct. Remember that w_hat stands for the changes in wages and not for the wages themselves.
#>
...
#< quiz "Exercise 3.1"
parts:
  - question: 1. In the whole model as CP build it, equation 6 stands for how many equations?
    sc:
    - 1
    - 2
    - 31
    ...
    - 1240*
    success: This is correct!
    failure: Try again with the help of the information in this exercise
  - question: 2. Why isn't there an exercise for equation 6 and an exercise for equation 7 instead of one exercise for the two of them together?
    sc:
    - Both single calculations are so short that each of the single exercises would be too small.
    - CP proceed the same way, without further explanation.
    - Calculations are too complicated and we skip them anyway.
    - Both equations are interrelated and we cannot evaluate them separately.*
    success: You're right!
    failure: Sorry, this is not the most relevant answer. Try again
  - question: 3. Consider two countries with a rather different state of the economic development (e.g. Mexico and the U.S.). What would CP's guess for the respective parameters 'P_hat' at the beginning of the iteration be?
    sc:
    - They would randomize the guess.
    - They would guess that P_hat for the U.S. is significantly higher than P_hat for Mexico.
    - They would guess that P_hat for the U.S. is a little smaller than P_hat for Mexico.
    - They would assume that P_hat equals 1 for both countries.*
    success: Great, this is the correct answer!
    failure: Sorry, this is not correct. Remember that P_hat stands for the change in prices and not for the prices themselves.
  - question: 4. When they find values for the costs (equation 6), how do CP proceed afterwards?
    sc:
    - They directly calculate the expenditure shares.
    - They directly calculate the prices.
    - They guess wages.
    - They check if equation 10 holds. If this is not the case, they guess new costs. They repeat this procedure in a loop until an equilibrium is found.
    - They check if equation 7 holds. If this is not the case, they guess new prices and calculate the corresponding values for the costs. They repeat this procedure in a loop, until an equilibrium is found.*
    success: Great, your answer is correct!
    failure: Sorry, this is not correct. Have a look again on the explanations above.
#>
...
#>
  ...
  piprime
#< add_to_hint
  display("1: As we saw above, cphat's variable costs needs to be arranged such that cphat's factor Country fits to ... ???
2: As we saw above, cphat's variable prices needs to be arranged such that cphat's factor Country fits to... ???")
#>
```
...
#>
  piprime <- mutate(piprime, 
                piprime =
                  Expend_Shares*
                  (((1+Tariffs2005/100)/(1+Tariffs1993/100)*costs/prices)^-Trade_Elast))
  piprime
#< add_to_hint
  display("In the formula, we want to multiply the changes in tariffs with costs and divide by prices, right?")
#>  
```
...
#< quiz "Equation 9"
parts:
  - question: 1. Does equation 9 calculate...
    sc:
    - changes in expenditure shares?
    - new import values?
    - the new sum of material expenditures plus households' expenditures?*
    - original expenditures?
    - changes in households' incomes?
    success: Perfect, this is correct!
    failure: Sorry, this is not correct, try again
  - question: 2. Advantages of matrix calculations over data frame calculations are...
    mc: 
    - the possibility to find results for 1240 equations at the same time.
    - the much higher calculation speed in programs like R and Matlab.*
    - the possibility to invert matrices to solve a system of 1240 equations.*
    - the ease to implement loops.
    success: Great, you are right!
    failure: Try again.
#>
...
#< quiz "Exercise 3.4"
question: What is the aim of this step of the equilibrium finding process?
mc:
  - Checking whether equation 10 holds.*
  - Calculating the equilibrium price.
  - Checking whether the outputs of the preceding steps in this iteration imply labour market clearing.*
  - Calculating a new guess of wages, if necessary.*
  - Checking if the equilibrium has been found.*
success: Yes, indeed!
failure: Sorry, you haven't found all correct answers yet.
#>
...
#< info "More on the welfare formula"
Remember that we introduced the composition of total income in Exercise 2.5 c) as follows:  
The total income of country $n$'s households $I_n$ (or simply country $n$'s income) is the sum of labour income $\mathsf{w}_n L_n$, trade deficit $D_n$ and tariff revenues $R_n$, thus $$I_n=\mathsf{w}_n L_n + D_n+ R_n.$$ 

In equation **(2)** in Exercise 2.1, we described the price of sector $j$ composite intermediate goods in country $n$ as
$$\tag{2} P^j_n = \Upsilon^j \left[ \sum^N_{i=1} \lambda^j_i (c^j_i \kappa^j_{ni})^{-\theta^j} \right]^{-\frac{1}{\theta^j}},$$
which is both the price of materials and the final consumption goods price. 
CP calculate the consumption price index employing Cobb-Douglas preferences as
$$P_n = \prod^J_{j=1} \left( \frac{P^j_n}{\alpha^j_n} \right)^{\alpha^j_n}, $$
whereat $\alpha^j_n$ is the final consumption share (see Exercise 2.4 e).  
Cobb-Douglas preferences imply that final consumption shares stay constant if prices stay constant. Thus a changing income does not affect consumption shares.
#>
...
#< quiz "Exercise 4.1"
parts:
  - question: 1. The welfare effects in Canada and the U.S. due to NAFTA tariff changes only are...
    sc:
    - highly positive.
    - almost zero.*
    - highly negative.
    success: Very good!
    failure: Try again.
  - question: 2. The effects that CP calculate should be considered...
    mc:
    - to hold exactly.
    - to be exaggerated.
    - to be only the effects of NAFTA tariff changes and not to be all the effects NAFTA induced.*
    - to give merely the order of magnitude of effects.*
    success: Great, this is correct!
    failure: Sorry, you did not find all correct answers. Try again.
  - question: 3. The welfare effects of only NAFTA tariff reductions on non-NAFTA members are...
    mc:
    - always negative.
    - even smaller than for NAFTA members in absolute figures.*
    - non-existent. 
    - beneficial for some few non-NAFTA members.*
    success: Great, you are right!
    failure: Sorry, not all answers are correct. Try again.
  - question: 4. Are there sectors that had a negative impact on U.S. welfare?
    sc:
    - Yes.
    - No.*
    - Don't know.
    success: Great!
    failure: There was no sector with a negative impact. Both the terms of trade and volume of trade effects are positive for the U.S. and every sector has a positive contribution to both effects.
  - question: 5. What is the electrical sector's total effect on Mexican welfare? Give your answer in percent, with at least two decimal places. 
    answer: 0.602344
    roundto: 0.01
    success: Great, well done!
    failure: The correct answer lies between 0 and 1. In the first code chunk in this exercise, you find the terms of trade and volume of trade effects for Mexico. In the fourth chunk you find the Electricals' contributions to these effects. Simply multiply and add the two products.
  - question: 6. By how many percent did Canadian exports to Mexico increase due to NAFTA tariff reductions?
    answer: 117
    success: Perfect!
    failure: Try again. Note that Canadian exports are given in the third column in the last code chunk.
#>
...
```{r "4.2 5", fig.width=12, fig.height=9}
#< task
...
#< quiz "Exercise 4.2"
parts:
  - question: 1. The effect on welfare and on wages of the NAFTA members due to NAFTA tariff changes, how did it vary from the model without worldwide tariff changes, to the model considering all of the changes?
    sc:
    - It did not vary at all.
    - It leads to very different results.
    - Effects increased significantly.
    - Effects lowered a little bit.*
    - Welfare increased while wages decreased.
    success: You are right, perfect!
    failure: Sorry, this is not correct, try again.
  - question: 2. For most of the countries in the sample, which effect was more important for their increase in welfare? 
    sc: 
    - The increase in wages.
    - The lower unemployment.
    - The volume of trade effect.*
    - The term of trade effect.
    success: You're right!
    failure: This is not correct, try again!
  - question: 3. What is the aggregate export share of the four most important Mexican sectors, after NAFTA came into effect?
    answer: 61.77369
    roundto: 1
    success: Great!
    failure: Sorry, this is not correct. Try again. Give the answer in percent.
  - question: 4. In contrast, what is the aggregate export share percentage of the four most important U.S. sectors?
    answer: 35.42581
    roundto: 1
    success: Well done!
    failure: Sorry, this is not correct. Try again. You may round to whole percent.
  - question: 5. Why are import volume effects so much smaller between Canada and the USA, in comparison to trade with Mexico?
    sc: 
    - Mexico is no industrial nation.
    - Canada and the USA share a larger border.
    - Canada and the USA already had very low mutual tariffs before NAFTA.*
    - Mexico's sectoral concentration of exports is much higher.
    success: Right!
    failure: Sorry, this is not correct. Give it another try.
  - question: 6. After all, tariff reductions in the course of NAFTA had...
    mc: 
    - high effects on wages?
    - high effects on trade volume?*
    - high effects on welfare?
    - low effects on trade volume?
    - moderate to very low effects on welfare?*
    - no effects on wages?
    - moderate to low effects on wages?*
    - negative effects on trade volume?
    success: Great!
    failure: These are not all the answers as proposed here. One could argue on the exact terminology of magnitude of effects. You may try again to hit the answers as proposed here.
#>
...
## enter your command here
#>
trade_elasticities(dest="Canada", sect="Petroleum", sour="USA", 
                   percentage_change = -10, trade=trade)
#< hint
display("You may pretty much exactly use the commands of the chunk above. But do not assign again the data frame 'trade'.
Copy-paste the line with 'trade_elasticities(....)' and substitute 'Chemicals' by 'Petroleum'.")
#>
```
...
#< quiz "Sensitivity of expenditure shares to Theta"
parts:
  - question: 1. The effect of the tariff reduction in the petroleum sector is roughly how many times higher than the effect of the same reduction in the chemical sector?
    answer: 100
    roundo: 3
    success: Great, the answer is correct!
    failure: This is not correct. Try again. Just give a multiple of ten as the answer.
  - question: 2. Which of the following statements are true?
    mc: 
      - Lower tariffs on products from one country lead to a higher market share of that country.*
      - A large Theta implies that trade relations within a sector are generally high. 
      - A large Theta implies that import shares react strongly on changes in tariffs in a sector.*  
      - A large Theta implies that the level of tariffs within a sector are generally high.
      - A large Theta implies that import shares react strongly on changes in prices in a sector.*
    success: Great, the answers are correct!
    failure: These are not all correct answers. Try again!
#>
...
#< quiz "EK and Theta"
question: What does a low Theta stand for?
mc: 
  - A low sectoral trade elasticity.*
  - A low comparative advantage in this sector.
  - Low geographic barriers. 
  - Relative production technologies are not very similar in this sector.*
  - It is more likely that some producers are far more efficient than others and can thus export more easily.*
  - It is less likely that some producers are far more efficient than others and can thus export more easily.
  - A high price sensitivity of demand.
success: Great, the answers are correct!
failure: These are not all correct answers. Try again!
#>
...
#>
trade %>%
  select(Sector,Trade_Elast) %>%
  filter(Sector=="Agriculture") %>%
  distinct()
#< add_to_hint
display("1: You want to select the columns 'Sector' and 'Trade_Elast'. What could be the name of the corresponding function?
2: Now you want to find all rows whose sector is agriculture. We performed such data processing many times before.")
#>
```
...
#< quiz "Estimating Theta"
parts:
  - question: 1. See in the task description. 
    answer: 9.11
    roundo: 0.1
    success: Great, the answer is correct!
    failure: This is not correct. Try again. Maybe you need to give more decimal places.
  - question: 2. What is the trade elasticity in the agricultural sector? 
    answer: 9.11
    roundo: 0.1
    success: Great, the answer is correct!
    failure: This is not correct. Is there actually any difference between the Thetas and the trade elasticities in this model?
  - question: 3. How do EK and CP get their values of trade elasticities?
    sc: 
      - They get them from existing literature.
      - There are free data bases who frequently publish the sectoral values.
      - They calculate them directly from the other data they employ.
      - They estimate them via the ordinary least squares (OLS) approach.*
    success: Great, the answer is correct!
    failure: This is not correct. Try again
  - question: 4. For the regression, CP can simply use tariff-expenditure pairs. Is that true?
    sc:
      - Yes, indeed. Still they do it in a more complicated way in order to need less computer calculation resources.
      - Yes, indeed, and they do it exactly like that.
      - No, they have to use a more complicated approach in order to cancel out some hindering terms.*
    success: Great, this is correct!
    failure: Sorry this is not the correct answer. Try again.
#>
...
```{r "5.1 4", optional=TRUE, points=0}
#< task_notest
# Enter code freely here
#>
...
#< info "CP and the implications from Pelkmans et. al (2014)"
Let us think about CP's methodology in the context of the paragraph above: Admittedly, it is very difficult estimating NTBs. Nevertheless, both sectoral competitiveness and NTBs, which are expressed by $d^j_{ni}$ in CP's model, determine to a large extent sectoral reactions on a free trade agreement. CP's parameter $\lambda^j_n$, as a notion of absolute advantage, stands for sectoral competitiveness relative to other countries, see Exercise 2.1.   
CP do not address the issue of changing NTBs in the course of a free trade agreement and how sectors would react to the changes. They explicitly do not calculate their NTB parameter $d^j_{ni}$, their absolute advantage parameter $\lambda^j_n$ and the parameters' respective changes and sensitivities. Still, despite the simplifications CP make their model is not simple. Hence, this finding seems to support Pelkmans et. al (2014) judging a proper and comprehensive TTIP analysis to be extremely difficult, if not infeasible.
#>
